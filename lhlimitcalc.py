...
def get_limit_lh(dm_pars: np.array,  # 1D array of the the DM masses
    ...
    for p in tqdm(dm_pars):
        ...
        # try:
        res_excl = bisect(implfunc, a=res_best.x[0], b=1e9,
                          maxiter=1000)  # TODO put b maybe to 2*best fit?? and only if this doesnt work make upper bound 10 times higher
